    ...
    <script>
        ...
        function handleRestore(uuid)
        {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ config('app.url') }}klien/restore/${uuid}`,
                    method: `POST`,
                    data: {
                        _token: `{{ csrf_token() }}`,
                        _method: `PUT`,
                    ...
                    success: function(result) {
                        if (result.icon == 'success') {
                            Swal.fire({
                                ...
                                if (res.isConfirmed) {
                                    window.location.href = `{{ config('app.url') }}klien/restorasi`
                                }
                            })
                        }
                    },
        ...
        function handleHapusPermanen(uuid)
        {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ config('app.url') }}klien/hapus-permanen/${uuid}`,
                    method: `POST`,
                    data: {
                        _token: `{{ csrf_token() }}`,
                        _method: `DELETE`,
