                ...
                >
                    <thead class="bg-slate-400">
                        <tr>
                            ...
                            <th class="py-3 px-4 uppercase font-semibold text-sm">Peran</th>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="py-3 px-4">
                                @foreach ($peran as $p)
                                @if ($pengguna->peran === $p['id'])
                                <span class="p-1 bg-slate-300 rounded">{{ $p['name'] }}</span>
                                @break
                                @endif
                                @endforeach
                            </td>
    ...
    <script>
        ...
        function handleHapus(uuid)
        {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ config('app.url') }}pengguna/hapus/${uuid}`,
                    method: `POST`,
                    data: {
                        _token: `{{ csrf_token() }}`,
                        _method: `DELETE`,
